...
A local-first DeepSeek usage and balance monitoring dashboard built with React, TypeScript, Vite, Tailwind CSS, FastAPI, and a native SwiftUI Apple app for iPhone and Mac.

一个本地优先的 DeepSeek API 用量与余额监控仪表盘，包含 React/FastAPI Web 版本，以及面向 iPhone 和 Mac 的 SwiftUI 原生 Apple App。
...
## Native Apple App Preview / 原生 Apple App 预览

<table>
  <tr>
    <td width="58%">
      <img src="./docs/assets/demo-dark.png" alt="macOS dashboard preview" width="760" />
    </td>
    <td width="42%">
      <img src="./docs/assets/native-ios.png" alt="iPhone dashboard preview" width="280" />
    </td>
  </tr>
  <tr>
    <td><strong>macOS</strong>: adaptive two-column dashboard with a large balance dial, status cards, model usage, and recent request panels.</td>
    <td><strong>iPhone</strong>: local SwiftUI dashboard with Keychain-stored API key, vertical layout, focus mode, and safe mock fallback when no key is configured.</td>
  </tr>
</table>

![Native focus mode preview](./docs/assets/demo-focus.png)

Native app screenshots use demo/mock data only. API keys are never shown and are not stored in this repository.

原生 app 截图仅使用演示/mock 数据，不展示真实 API key，也不会把密钥保存到仓库中。

## English

### Overview

DeepSeek Usage Monitor is a local dashboard for viewing DeepSeek API balance, estimated spend, request activity, token usage, and model-level usage breakdowns. It is designed for developers who want a focused monitor while keeping credentials on their own machine.

The repository contains two app surfaces:

- Web app: Vite + React frontend with a local FastAPI proxy.
- Native Apple app: pure SwiftUI app for macOS and iPhone, plus an iOS WidgetKit Home Screen widget.

This project is not positioned as a production-ready billing system. Data can come from the local backend, the native app, local settings, or built-in mock/demo data depending on how it is started and configured.
...
An experimental pure SwiftUI Apple app lives in `AppleApp/`. It can be developed and run locally without Node, Vite, Python, or FastAPI.

- Open `AppleApp/DeepSeekUsageMonitor.xcodeproj` in Xcode.
- Use the `DeepSeekUsageMonitor macOS` scheme to run the Mac app locally.
- Use the `DeepSeekUsageMonitor iOS` scheme to run the iPhone app in Simulator or on a signed iPhone device.
- The native app stores the DeepSeek API key in the device Keychain and stores non-secret preferences in `UserDefaults`.
- The native app calls the DeepSeek balance endpoint directly and keeps request/model/trend panels as safe mock data for the first native version.
- macOS: adaptive desktop layout with a large balance dial, settings sheet, light/dark/system themes, bilingual copy, and responsive one-column fallback for narrower windows.
- iPhone: vertical dashboard optimized for portrait use, settings sheet, focus mode for dial-only monitoring, and safe mock fallback when no API key is configured.
- iOS Widget: WidgetKit Home Screen widget extension, `DeepSeekUsageMonitorWidgetExtension`, with a square dashboard-style widget.
- The widget reads the API key from a shared Keychain access group, fetches the live DeepSeek balance when WidgetKit asks for a new timeline, writes a non-secret snapshot to the App Group `group.com.local.DeepSeekUsageMonitor`, and the main app also asks WidgetKit to reload after each app refresh.
- The iOS app also schedules a `BGAppRefreshTask` (`com.local.DeepSeekUsageMonitor.refresh`) so iOS can refresh the balance in the background, save a fresh snapshot, and reload the widget without manually opening the app.
- For real iPhone installation, the iOS app target and widget extension must use the same signing team, the same App Group capability, and the same Keychain Sharing group `$(AppIdentifierPrefix)com.local.DeepSeekUsageMonitor.shared`; the checked-in project currently uses Team ID `V6PB5KA8AG`. Open the app once after installing and save the API key so it is written to the shared Keychain group, and keep iOS Settings → General → Background App Refresh enabled for this app. WidgetKit and BackgroundTasks may still throttle background refreshes when the app is not running; true push-level real-time updates require a server plus WidgetKit push/APNs.
...
DeepSeek Usage Monitor 是一个本地优先的 DeepSeek API 用量与余额监控仪表盘，用于查看账户余额、估算消费、请求活动、Token 用量以及模型维度的使用占比。它更适合开发者在本机做日常观察，而不是替代官方账单系统。

仓库包含两个使用入口：

- Web app：Vite + React 前端，配合本地 FastAPI 代理。
- 原生 Apple app：面向 macOS 和 iPhone 的纯 SwiftUI app，并包含 iOS WidgetKit 主屏幕小组件。

根据启动方式与配置不同，页面数据可能来自本地后端、原生 app、本地设置，或项目内置的 mock/demo 数据。
...
项目已新增实验性的纯 SwiftUI 原生 Apple App，位于 `AppleApp/`。原生 app 本身不再依赖 Node、Vite、Python 或 FastAPI 运行。

- 在 Xcode 中打开 `AppleApp/DeepSeekUsageMonitor.xcodeproj`。
- 使用 `DeepSeekUsageMonitor macOS` scheme 在 Mac 本机运行 macOS app。
- 使用 `DeepSeekUsageMonitor iOS` scheme 在 iPhone 模拟器或已签名真机上运行。
- 原生 app 将 DeepSeek API 密钥保存到本机 Keychain，其他非敏感偏好保存到 `UserDefaults`。
- 原生 app 会直接请求 DeepSeek 余额接口，首版仍保留请求/模型/趋势面板为安全 mock 数据。
- macOS：自适应桌面双栏布局，包含大尺寸余额仪表盘、设置 sheet、深浅色/系统主题、中英文文案，以及窄窗口单栏回退。
- iPhone：适配竖屏的纵向仪表盘布局，包含设置 sheet、只显示仪表盘的 Focus 模式，以及无 API key 时的安全 mock fallback。
- iOS Widget：内嵌 WidgetKit 主屏幕小组件扩展 `DeepSeekUsageMonitorWidgetExtension`，提供正方形仪表盘风格的小组件。
- Widget 会从共享 Keychain 访问组读取 API key，在 WidgetKit 请求新时间线时直接获取 DeepSeek 真实余额，并把不含密钥的快照写到 App Group `group.com.local.DeepSeekUsageMonitor`；主 app 每次刷新后也会通知 WidgetKit 重新加载。
- iOS app 还会排程一个后台刷新任务 `BGAppRefreshTask`（`com.local.DeepSeekUsageMonitor.refresh`），让系统允许时在后台拉取余额、保存快照并刷新 widget，不需要手动打开 app。
- 真机安装时，iOS app target 和 Widget extension 需要使用同一个签名 Team、同一个 App Group capability，以及同一个 Keychain Sharing group：`$(AppIdentifierPrefix)com.local.DeepSeekUsageMonitor.shared`；当前项目里的 Team ID 是 `V6PB5KA8AG`。安装后请打开 app 一次并重新保存 API Key，让它写入共享 Keychain，并确认 iOS 设置 → 通用 → 后台 App 刷新 已允许这个 app。app 不运行时，WidgetKit 和 BackgroundTasks 仍会按系统策略限制后台刷新频率；如果要做到推送级实时刷新，需要额外服务器配合 WidgetKit push/APNs。
